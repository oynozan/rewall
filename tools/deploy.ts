...
const salt = (kind: string, scope: string) =>
    BigInt(
        keccak256(
            encodeAbiParameters([{ type: "bytes32" }, { type: "string" }], [keccak256(stringToHex(kind)), scope]),
        ),
    );
...
if (await deployed(entry.namespaceRegistry)) {
    ...
    const { address, gas } = await deployProxy(
        send,
        USER_REGISTRY_IMPL,
        "rewall.registry.v1",
        `${NAMESPACE_LABEL}.${owner.label}`,
        initData,
    );
...
const nsSub = await publicClient.readContract({
