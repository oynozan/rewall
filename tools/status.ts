...
for (const p of PARTICIPANTS) {
    ...
    const usdc = await read(
        client.readContract({ address: MOCK_USDC, abi: erc20Abi, functionName: "balanceOf", args: [address] }),
    );
    ...
    const tokenId = await read(
        client.readContract({ address: ETH_REGISTRY, abi: registryAbi, functionName: "getTokenId", args: [labelhash] }),
    );
    const nameOwner =
        tokenId === null
            ? null
    ...
    const expiry = await read(
        client.readContract({ address: ETH_REGISTRY, abi: registryAbi, functionName: "getExpiry", args: [labelhash] }),
    );
    const resolver = await read(
        client.readContract({ address: ETH_REGISTRY, abi: registryAbi, functionName: "getResolver", args: [p.label] }),
    );
    const subregistry = await read(
        client.readContract({
            address: ETH_REGISTRY,
            abi: registryAbi,
    ...
    console.log(
        `  held by      ${nameOwner ?? "nobody"}${nameOwner && nameOwner.toLowerCase() === address.toLowerCase() ? "  matches" : nameOwner ? "  MISMATCH" : ""}`,
    );
...
const projectName = await read(
    client.readContract({ address: ETH_REGISTRAR, abi: registrarAbi, functionName: "isAvailable", args: ["rewall"] }),
);
console.log(
