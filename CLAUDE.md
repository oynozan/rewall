...
- `sdk/` publishes to npm as `@rewall/sdk`. `web/` and `mcp/` depend on the published version, so a deploy of either needs nothing from `sdk/`. `examples/`, `extension/`, `ledger/`, `rail/` and `tools/` still share it by relative link, `"@rewall/sdk": "link:../sdk"`, and those must have `pnpm build` run in `sdk/` first, since a link imports `dist`. A change the dashboard or the MCP server has to see is a publish, not a rebuild.
