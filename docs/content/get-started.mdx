...
The Recovery page walks a replacement wallet through recovering a vault protected by guardians.
Connect the new wallet, unlock it, type the lost vault's name, and share the link it gives you with
your guardians. Each guardian opens the link and clicks Approve, which publishes their piece on their
own name as `rewall.reshare.<fingerprint>`. When enough have approved, Recover this vault rebuilds
the key and lets you Reveal each secret. Then publish a new key and rotate every secret.

Guardians are set up with the SDK today, and the dashboard has no screen yet that takes the 24
words from the wizard. The words are an ordinary wallet phrase. With the SDK,
...
built with that `identity` can `get` each secret. See
[example 5](/examples/05-recover-a-lost-wallet) for guardians.
