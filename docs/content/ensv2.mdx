...
compares the signer to its owner. Storing a secret is therefore one transaction, and a create by any
wallet other than that owner is refused before anything is written. The SDK never gives a secret a
