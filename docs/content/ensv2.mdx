...
same bit shifted left by 128. Rewall uses `bigint` for them. A secret subname is registered with
`SET_RESOLVER` and its admin variant and nothing else, because a secret needs no children.
...
From then on the SDK mints secrets by itself. `ensureSecretName` in `sdk/src/client.ts` asks
`findParentRegistry` which registry holds `openai.rewall.alice.eth`, then checks `getResolver` for
the label. If that is empty it calls `register` with the caller as owner, no subregistry, the
parent's resolver, the `SET_RESOLVER` roles and the parent's expiry. Registering happens before any
record is written, because a record on an unregistered subname reads back fine but has no owner, and
then nothing could ever rotate it.
