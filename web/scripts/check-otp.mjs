...
`,loader:'tsx',resolveDir:process.cwd()},bundle:true,write:false,format:'iife',platform:'browser',jsx:'automatic',define:{'process.env.NODE_ENV':'"production"'},logLevel:'silent'});
...
try {
 ...
 await page.route(origin+'/__component-test',route=>route.fulfill({contentType:'text/html',body:`<!doctype html><html><head>${styles}<style>.fixture{padding:32px}.fixture h1{font-size:20px}.fixture p{margin:6px 0 32px}.fixture-chart{margin-top:32px;max-width:520px}.fixture-chart .volume-plot{height:120px}</style></head><body class="${classes}"><div id="root"></div></body></html>`}));
 await page.goto(origin+'/__component-test');
 await page.clock.install({time:new Date(59000)});
 ...
 await copy.hover();await page.clock.runFor(200);
 await expect(page.locator('.otp-tooltip')).toHaveCSS('opacity','1');
 await copy.click();assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),'94287082');
 ...
 await copy.click();assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),expected.code);
 await expect(page.locator('.otp-digits')).toHaveAttribute('aria-hidden','true');
 await copy.focus();await expect(page.locator('.otp-tooltip')).toHaveCSS('opacity','1');
 await page.clock.runFor(2200);await page.evaluate(()=>document.fonts.ready);
 ...
 const checks=['RFC 6238 known code','Input buffers wiped on success and failure','HOTP rejected','Code remains blurred and hidden from accessible text','Copy tooltip on hover and keyboard focus','Real clock rollover changes code and resets timer','Copy uses the current code','Populated chart has separate sent/shared series','Mobile component has no page overflow'];
