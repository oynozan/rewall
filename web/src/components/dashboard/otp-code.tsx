        ...
        <div className="otp-live">
            <div className="otp-copy-wrap">
                <button className="otp-code" aria-label={`Copy code for ${label}`} onClick={copy} disabled={!snapshot}>
                    <span className="otp-digits mono" aria-hidden="true">{snapshot?.code.replace(/(.{3,4})(.{3,4})$/, "$1 $2") || "••• •••"}</span>
                    <span className="otp-tooltip" aria-hidden="true"><Glyph name="copy" size={20} /></span>
                </button>
                <span className="otp-copy-status" role="status">{status}</span>
            </div>
            <div className="otp-timer">
                <SegmentedProgress value={snapshot?.remaining || 0} max={otp.period} segments={20} label={`${label} code expires in ${snapshot?.remaining || 0} seconds`} />
